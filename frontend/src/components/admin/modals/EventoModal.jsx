...
export const EventoModal = ({ isOpen, onClose, onSave, onDelete, editingEvento }) => {
  const empty = { titulo: '', descripcion: '', fecha: '', tipo: 'actividad', importante: false };
  ...
  useEffect(() => {
    setForm(editingEvento ? {
      titulo:      editingEvento.titulo      || '',
      descripcion: editingEvento.descripcion || '',
      fecha:       editingEvento.fecha       || '',
      tipo:        editingEvento.tipo        || 'actividad',
      importante:  editingEvento.importante  || false,
    } : empty);
    setError('');
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={editingEvento ? 'Editar Evento' : 'Agregar Evento'}>
      <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
        <div>
          <label className="text-xs font-medium text-slate-600 mb-1 block">Título *</label>
          <input value={form.titulo} onChange={e => set('titulo', e.target.value)} required placeholder="Título del evento"
        ...
        <div className="flex items-center gap-2">
          <input type="checkbox" id="imp" checked={form.importante} onChange={e => set('importante', e.target.checked)}
            className="w-3.5 h-3.5 accent-[#3D52A0]"/>
          <label htmlFor="imp" className="text-xs text-slate-600 cursor-pointer">Marcar como importante</label>
        </div>
        ...
        <div className="flex items-center justify-between pt-1">
          {editingEvento ? (
            <button type="button" onClick={() => onDelete(editingEvento)}
          ...
          <div className="flex gap-3">
            ...
            <button type="submit" disabled={loading}
              ...
              {loading && <Spinner/>}{editingEvento ? 'Guardar' : 'Crear'}
            </button>
          </div>
        </div>
      </form>
