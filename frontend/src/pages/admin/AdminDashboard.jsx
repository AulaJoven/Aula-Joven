...
const AdminDashboard = () => {
  ...
  const [delGrupo, setDelGrupo] = useState({
    open: false,
    grupo: null,
    loading: false,
  });
  ...
  const saveUser = async (data) => {
    if (editingUser) {
      ...
      if (data.rol === "profesor")
        setProfesores((p) =>
          p.map((u) => (u.id === editingUser.id ? updated : u)),
        );
      else
        setEstudiantes((p) =>
          p.map((u) => (u.id === editingUser.id ? updated : u)),
      ...
      const newUser = res.data.data || { ...data, id: Date.now() };
      if (data.rol === "profesor") setProfesores((p) => [...p, newUser]);
      else setEstudiantes((p) => [...p, newUser]);
    }
  ...
  const confirmDeleteUser = async () => {
    ...
    try {
      await api.delete(`/admin/users/${delUser.user.id}`);
      await fetchAll();
      setDelUser({ open: false, user: null, loading: false });
    } catch {
      setDelUser((p) => ({ ...p, loading: false }));
    }
  };

  const saveGrupo = async (data) => {
    editingGrupo
      ? await api.put(`/admin/grupos/${editingGrupo.id}`, data)
      : await api.post("/admin/grupos", data);
    await fetchAll();
  };
  const confirmDeleteGrupo = async () => {
    setDelGrupo((p) => ({ ...p, loading: true }));
    try {
      await api.delete(`/admin/grupos/${delGrupo.grupo.id}`);
      await fetchAll();
      setDelGrupo({ open: false, grupo: null, loading: false });
    } catch {
      setDelGrupo((p) => ({ ...p, loading: false }));
    }
  };

  const saveEvento = async (data) => {
    editingEv
      ? await api.put(`/admin/eventos/${editingEv.id}`, data)
      : await api.post("/admin/eventos", data);
    await fetchAll();
  };
  const confirmDeleteEv = async () => {
    ...
    try {
      ...
      await fetchAll();
      setDelEv({ open: false, ev: null, loading: false });
    } catch {
      setDelEv((p) => ({ ...p, loading: false }));
    }
  };
      ...
      <main className="flex-1 max-w-6xl mx-auto w-full px-6 py-8">
        ...
        {activeTab === "grupos" && (
          <GruposTab
            ...
            setGrupoModal={setGrupoModal}
            editingGrupo={editingGrupo}
            setEditingGrupo={setEditingGrupo}
            delGrupo={delGrupo}
            setDelGrupo={setDelGrupo}
            inscModal={inscModal}
            setInscModal={setInscModal}
            inscGrupo={inscGrupo}
            setInscGrupo={setInscGrupo}
            onSave={saveGrupo}
        ...
        {activeTab === "calendario" && (
          <CalendarioTab
            eventos={eventos}
            onEventosChange={fetchAll}
            evModal={evModal}
            setEvModal={setEvModal}
            editingEv={editingEv}
            setEditingEv={setEditingEv}
            delEv={delEv}
            setDelEv={setDelEv}
            onSave={saveEvento}
            onConfirmDelete={confirmDeleteEv}
          />
        )}
      </main>
