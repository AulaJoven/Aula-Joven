// src/pages/admin/EstudiantesTab.jsx
import { useState, useMemo } from 'react';
import { EstudianteCard } from '../../components/admin/EstudianteCard';
import { UserModal } from '../../components/admin/modals/UserModal';
import { DeleteModal } from '../../components/admin/modals/DeleteModal';
import { BulkEstudiantesModal } from '../../components/admin/modals/BulkEstudiantesModal';
...
export const EstudiantesTab = ({
  ...
  const filtered = useMemo(() => {
    ...
    if (grado) list = list.filter(u => String(u.grado) === grado);
  ...
  return (
    <>
      ...
      <div className="bg-emerald-500 rounded-xl p-5 flex items-center justify-between mb-6 text-white">
        <div>
          <p className="text-xs text-white/70">Total Estudiantes</p>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-12 text-center">
          <p className="text-slate-400 text-sm">
            {hasFilters ? 'Sin resultados con los filtros aplicados.' : 'No hay estudiantes registrados aún.'}
          </p>
        </div>
      ) : (
        <>
      ...
      />
      <BulkEstudiantesModal
        isOpen={bulkModal}
        onClose={() => setBulkModal(false)}
        onSuccess={() => { setBulkModal(false); onRefresh?.(); }}
      />
    </>
  );
};